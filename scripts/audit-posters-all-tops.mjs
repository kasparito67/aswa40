...
const report={generatedAt:new Date().toISOString(),source:'ASWA40 runtime data + canonical poster metadata + established TMDB manifests',note:'TMDB candidates are only emitted for unresolved records; ambiguous records remain unresolved.',tops:{},unresolved:[],withoutTmdb:[]};
...
lines.push('','## Entries without canonical TMDB ID','');if(!report.withoutTmdb.length)lines.push('0 entry.');else for(const x of report.withoutTmdb)lines.push(`- ${x.topId} #${x.rank} — **${x.title}** — poster ${x.status}`);
