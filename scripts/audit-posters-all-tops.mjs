...
// Keep this load order aligned with v2/index.html so the audit sees the real runtime dataset.
const files=['scripts/data.js','scripts/backdrops.js','v2/scripts/data.js','v2/scripts/new-tops.js','v2/scripts/animation-fix.js','v2/scripts/expanded-tops.js','v2/scripts/poster-metadata.js','v2/scripts/poster-aliases.js','v2/scripts/top-skeletons.js','v2/scripts/hero-config.js','v2/scripts/section-polish.js'];
...
const report={generatedAt:new Date().toISOString(),source:'ASWA40 runtime data + canonical poster metadata + established TMDB manifests',note:'TMDB candidates are only emitted for unresolved records; ambiguous records remain unresolved.',tops:{},unresolved:[],withoutTmdb:[],withoutEntryTmdb:[]};
for(const top of tops.filter(t=>t.films?.length)){
  ...
  for(const film of top.films){
    ...
    const item={topId:top.id,rank:film.rank,title:film.title,img:src,status,reason,entryTmdbId:film.tmdbId||null,tmdbId:film.tmdbId||existing?.tmdbId||null,posterPath:film.posterPath||existing?.posterPath||null,backdropPath:film.backdropPath||existing?.backdropPath||null,year:film.year||existing?.tmdbDate?.slice?.(0,4)||null};
    if(status!=='ok'){item.candidates=await tmdbCandidates(film.title);report.unresolved.push(item)}
    if(!item.tmdbId)report.withoutTmdb.push({topId:item.topId,rank:item.rank,title:item.title,img:item.img,status:item.status});
    if(!item.entryTmdbId)report.withoutEntryTmdb.push({topId:item.topId,rank:item.rank,title:item.title,img:item.img,status:item.status,canonicalFallback:item.tmdbId||null});
    items.push(item);
  }
...
report.summary={total:Object.values(report.tops).reduce((n,t)=>n+t.total,0),validPoster:Object.values(report.tops).reduce((n,t)=>n+t.validPoster,0),missing:Object.values(report.tops).reduce((n,t)=>n+t.missing,0),invalid:Object.values(report.tops).reduce((n,t)=>n+t.invalid,0),withTmdbId:Object.values(report.tops).reduce((n,t)=>n+t.withTmdbId,0),withoutTmdbId:report.withoutTmdb.length,withoutEntryTmdbId:report.withoutEntryTmdb.length};
await fs.writeFile('poster-audit.json',JSON.stringify(report,null,2)+'\n');
const lines=['# ASWA40 — Poster audit','',`Generated: ${report.generatedAt}`,'',`Total films: **${report.summary.total}**  `,`Valid posters: **${report.summary.validPoster}**  `,`Missing placeholders: **${report.summary.missing}**  `,`Invalid assets/URLs: **${report.summary.invalid}**  `,`Films with canonical/verified TMDB IDs: **${report.summary.withTmdbId}**  `,`Entries without resolvable canonical TMDB ID: **${report.summary.withoutTmdbId}**  `,`Runtime entries without their own tmdbId field: **${report.summary.withoutEntryTmdbId}**`,'','## By Top',''];
...
lines.push('','## Entries without resolvable canonical TMDB ID','');if(!report.withoutTmdb.length)lines.push('0 entry.');else for(const x of report.withoutTmdb)lines.push(`- ${x.topId} #${x.rank} — **${x.title}** — poster ${x.status}`);
lines.push('','## Runtime entries without their own tmdbId field','');if(!report.withoutEntryTmdb.length)lines.push('0 entry.');else for(const x of report.withoutEntryTmdb)lines.push(`- ${x.topId} #${x.rank} — **${x.title}** — poster ${x.status}${x.canonicalFallback?` · canonical fallback ${x.canonicalFallback}`:''}`);
lines.push('','## Unresolved posters','');if(!report.unresolved.length)lines.push('0 poster missing.');else for(const x of report.unresolved){lines.push(`- ${x.topId} #${x.rank} — **${x.title}** — ${x.reason}`);for(const c of x.candidates||[])lines.push(`  - candidate: TMDB ${c.tmdbId} · ${c.title} (${c.year||'year ?'}) · poster ${c.posterPath?'yes':'no'}`)}
