...
const runtimeFiles=['scripts/data.js','scripts/backdrops.js','v2/scripts/data.js','v2/scripts/new-tops.js','v2/scripts/animation-fix.js','v2/scripts/expanded-tops.js','v2/scripts/poster-aliases.js','v2/scripts/top-skeletons.js','v2/scripts/hero-config.js','v2/scripts/section-polish.js'];
...
async function search(title){const aliases={'The Rolling Stones: Gimme Shelter':'Gimme Shelter'};const p=new URLSearchParams({query:aliases[title]||title,include_adult:'false',language:'en-US'});return (await api(`https://api.themoviedb.org/3/search/multi?${p}`)).results?.filter(x=>x.media_type==='movie'||x.media_type==='tv')||[]}
async function choose(top,film){const key=`${top.id}:${film.rank}`;if(overrides[key])return details(overrides[key]);if(editorialExceptions.has(key))return null;const results=await search(film.title);const aliases={'The Rolling Stones: Gimme Shelter':'Gimme Shelter'};const expected=aliases[film.title]||film.title;const exact=results.filter(x=>norm(x.title||x.name)===norm(expected)&&x.poster_path);return exact[0]||null}
