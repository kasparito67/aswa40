...
async function choose(top,film){const key=`${top.id}:${film.rank}`;if(overrides[key])return details(overrides[key]);if(editorialExceptions.has(key))return null;const results=await search(film.title);const exact=results.filter(x=>norm(x.title)===norm(film.title)&&x.poster_path);return exact[0]||null}
